...
app.get("/collection/:id", function(page, model, params, next) {
  ...
  var artistList = [];
  model.subscribe(artistQuery, collectionAt, function(err) {
    if (err) return next(err);
    ...
    //console.log("collection=%j", collectionAt.get());
    model.set('_page.artist', artistList);
...
//app.collectionAddArtist = function(model) {
exports.collectionAddArtist = function(model, e, el, next) {
  console.log(model.get("_page.collectionOption.selecteda"));
