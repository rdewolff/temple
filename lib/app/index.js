...
app.artistListForDropdown = function() {
  var model = this.model;
...
app.get("/collection/:id", function(page, model, params, next) {
  ...
  if (params.id == "new") {
    var collection = model.at('collection');
  } else {
  ...
  console.log("val: %j", app.artistListForDropdown());
  model.set('_page.artist', app.artistListForDropdown); // model containing the artist
  page.render('collection');
});
