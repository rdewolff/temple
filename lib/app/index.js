...
module.export = function isEmpty(obj) {
...
app.get("/search/:query?", function(page, model, params, next) {
  console.log("SEARCH/QUERY with param=%j",params.query);

  //if (typeof params.query === 'undefined' || params.query == "{}") {
  if (params.query == null || params.query.length = 0) {
    model.set("_page.filter", "");
    console.log("_page.filter is empty");
  } else {
    model.set("_page.filter", params.query);
    console.log("_page.filter is FILLED");
  }
