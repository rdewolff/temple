...
app.get("/collection/:id", function(page, model, params, next) {
  ...
  var artistList = [];
  artist.fetch(function(err){
    if (err) return next(err);
    console.log("artist=", artist.get());
    for (var item in artist.get()) {
    ...
    collection.subscribe(function(err) {
      if (err) return next(err);
      console.log("artist=", artist.get());
      console.log("collection=", collection.get());
      console.log(new Date());
      model.set('_page.artist', artist.get());
  ...
  var artistList = [];
  model.fetch(artist, function(err) {
    if (err) return next(err);  
    // console.log("artist: %j", artist); // circular error
